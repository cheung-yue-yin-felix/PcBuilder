...
import { hasCompleteRange, type PagedRequest, type PagedResult, type RangeFilter } from './paging'

export type DdrGeneration = 'Ddr4' | 'Ddr5'
export type RamRank = 'SingleRank' | 'DualRank'
...
export function listCpus(params: CpuListParams) {
  ...
  if (!isCpuFilterActive(params.filter)) {
    return api.get<PagedResult<CpuListItem>>('/catalog/cpu', { params: paging }).then((response) => response.data)
  }
...
export function getCpuById(id: string) {
  return api.get<CpuDetail>(`/catalog/cpu/${id}`).then((response) => response.data)
}
