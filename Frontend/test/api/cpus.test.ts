...
import { getCpuById, isCpuFilterActive, listCpus } from '@/api/cpus.ts'
...
describe('cpu API', () => {
  ...
  it('lists with GET when no filter is set', async () => {
    get.mockResolvedValue({ data: { items: [item], totalCount: 1, pageIndex: 0, pageSize: 10 } })
    await expect(listCpus({ ...paging, filter: { name: '' } })).resolves.toMatchObject({
      items: [item],
    })
  ...
  it('lists with POST /query when a filter is set', async () => {
    post.mockResolvedValue({ data: { items: [item], totalCount: 1, pageIndex: 0, pageSize: 10 } })
  ...
  it('does not send ddrGeneration on the query body', async () => {
    post.mockResolvedValue({ data: { items: [], totalCount: 0, pageIndex: 0, pageSize: 10 } })
    await listCpus({
      ...paging,
  ...
  it('loads a CPU by id', async () => {
    get.mockResolvedValue({ data: { ...item, ramCompats: [], supportChipsets: [] } })
