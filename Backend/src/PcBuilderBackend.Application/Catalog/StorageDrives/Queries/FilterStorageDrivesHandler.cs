...
public class FilterStorageDrivesHandler(IApplicationDbContext context, IMapper mapper)
    : IRequestHandler<FilterStorageDrivesQuery, PagedResult<StorageDriveDto>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var queryable = context.StorageDrives.AsNoTracking()
        ...
        if (filter.MotherboardId is { } motherboardId)
        {
            ...
            if (motherboard is null)
                return PagedResult<StorageDriveDto>.Empty(req);
        }
        ...
        // Domain Check*Storage*Compatibility is not EF-translatable; filter in memory then page.
