...
public class ActiveEntityGuardTests : IDisposable
{
    ...
    {
        await ActiveEntityGuard.EnsureManufacturersExist(_fx.Lookup, [], CancellationToken.None);
        await ActiveEntityGuard.EnsureManufacturersExist(_fx.Lookup, [_fx.Manufacturer.Id], CancellationToken.None);
    }
    ...
    {
        await ActiveEntityGuard.EnsureSocketsExist(_fx.Lookup, [_fx.Socket.Id], CancellationToken.None);
        await ActiveEntityGuard.EnsureChipsetsExist(_fx.Lookup, [_fx.Chipset.Id], CancellationToken.None);
        await ActiveEntityGuard.EnsureGpusExist(_fx.Lookup, [_fx.Gpu.Id], CancellationToken.None);
    }
}
